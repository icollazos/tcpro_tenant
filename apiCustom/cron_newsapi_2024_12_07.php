...
foreach ($palabras as $k) {
    ...
    while ($_SESSION['page']!='FIN') {
        ...
        $q=$q.$page;            
       //$apikey="pub_5999094320976f0c3bfbdacc9fc90e5ed30ad";
        $from_date="&from_date=2024-09-01";

        $n = 90; // Cambia este valor según tus necesidades
        $from_date = date("Y-m-d", strtotime("-$n days"));
        $from_date = "&from_date=".$from_date;

        $url = "https://newsdata.io/api/1/archive?apikey=".$apikey.$from_date.$q;
        pre($url);
