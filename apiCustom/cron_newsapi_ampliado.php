...
foreach ($palabras as $k) {
	...
	switch ($k['idfuente']) {
        ...
        $url = "https://newsapi.org/v2/everything?apiKey=5999094320976f0c3bfbdacc9fc90e5ed30ad&language=es&q=$query";
        $url = "https://newsapi.org/v2/everything?apiKey=pub_5999094320976f0c3bfbdacc9fc90e5ed30ad&language=es&q=$query";
        $url = "https://newsdata.io/api/1/news?apikey=pub_5999094320976f0c3bfbdacc9fc90e5ed30ad$query";
...
function extraeTextosNewsApi($array){
	...
	foreach ($array as $key => $value) {
        ...
        $z[$i]['idaaa_source']=$value['source']['id'];
        $z[$i]['source']=$value['source']['name'];
        $z[$i]['author']=$value['author'];
        $tl = strtr($tc, $mapa);
        $z[$i]['texto']=$tc;
        $z[$i]['url']=$value['url'];
        $z[$i]['urlToImage']=$value['urlToImage'];
        $z[$i]['urlToImage']=$value['urlToImage'];
        $z[$i]['publishedAt']=substr($value['publishedAt'],0,10);
        $z[$i]['textoLimpio']=$tl;
        $i++;
    }
    return $z;
...
function cargaTextoIndividual($conn, $registro, $iditem) {
    ...
    try {
        $idaaa_source=1;
        $sql = "INSERT INTO aaa_texto (controlador, descriptor, textolimpio, idaaa_item, fecha, analizado, url, longitud, urlsource, urltoimage) VALUES ('$controlador', '$texto', '$textoLimpio', '$iditem', '$fecha', '$analizado', '$urlTexto', '$longitud', '$b1', '$urlToImage')";  
        $r[]=$sql;
        $stmt = $conn->prepare($sql);
    	/*
