...
function ejecutarRobot($conn){
    ...
    $sql = "DELETE FROM aaa_textovalor WHERE roh='R' AND aaa_textovalor.id IN (SELECT v_aaa_textovalor.id FROM v_aaa_textovalor WHERE v_aaa_textovalor.id_variable_valor = '$idvariable' )" ;
    $conn->exec($sql); 
    $sql = "SELECT DISTINCT(aaa_dicrechazo.lemapar) AS lemapar, aaa_valor.id AS idvalor, aaa_valor.descriptor AS valor FROM aaa_dicrechazo INNER JOIN aaa_valor ON aaa_valor.id = aaa_dicrechazo.idaaa_valor WHERE aaa_valor.idaaa_variable = $idvariable AND aaa_dicrechazo.lemapar IS NOT NULL;";
    ...
    $sql = "SELECT DISTINCT(aaa_dicValor.lemapar) AS lemaPar, aaa_valor.id AS idValor, aaa_valor.descriptor AS valor FROM aaa_dicValor INNER JOIN aaa_valor ON aaa_valor.id = aaa_dicValor.idaaa_valor WHERE aaa_valor.idaaa_variable = $idvariable AND aaa_dicvalor.lemapar IS NOT NULL ";
    $stmt = $conn->prepare($sql);
    $i = 0;
    if ($stmt->execute()) {
        $valores = [];
    ...
    $r[]=$valores;
    $sql = "SELECT DISTINCT(aaa_dicTextoValor.lemapar) AS lemapar, aaa_valor.id AS idvalor, aaa_valor.descriptor AS valor FROM aaa_dictextovalor     INNER JOIN aaa_textovalor ON aaa_textovalor.id = aaa_dictextovalor.idaaa_textovalor INNER JOIN aaa_valor ON aaa_valor.id = aaa_textovalor.idaaa_valor WHERE aaa_valor.idaaa_variable = $idvariable AND aaa_dicTextoValor.lemapar IS NOT NULL;";
    $stmt = $conn->prepare($sql);
    $i = 0;
    if ($stmt->execute()) {
        //$valores = [];
